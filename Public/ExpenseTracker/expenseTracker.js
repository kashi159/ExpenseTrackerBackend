...
async function isPremium(){
    try{
        const user = await axios.get("http://3.222.129.220:80/user/status", { headers: {"Authorization" : token }})
        // console.log(user)
        if(user.data === true){
...
async function showLeaderBoard(e){
    ...
    try{
        ...
        const users = await axios.get("http://3.222.129.220:80/premium/leadershipboard", { headers: {"Authorization" : token }});
        // console.log(users);
...
async function showTotalExpense() {
   ...
   try{
   const response = await axios.get("http://3.222.129.220:80/user/expense", { headers: {"Authorization" : token }})
        // console.log(response)
...
async function onSubmit(e) {
    ...
    try{
        const response = await axios.post('http://3.222.129.220:80/user/expense', userExpense,{
             headers: {
                "Authorization" : token 
            }
        });
    ...
    try {
        if(e.target.classList.contains('delete')){
            if(confirm('Are You Sure?')){
                ...
                id = li.id;
                await axios.delete(`http://3.222.129.220:80/user/delete/${id}`, { headers: {"Authorization" : token }})
                expense.removeChild(li);
                showTotalExpense();
            }
            }
   ...
   //Edit item//
    ...
    async function editUser(e) {
        try{
            if(e.target.classList.contains('edit')){
                var li=e.target.parentElement;
                id = li.id;
                const response = await axios.get(`http://3.222.129.220:80/user/edit/${id}`, { headers: {"Authorization" : token }});
                console.log(response)
                expense.removeChild(li);
    ...
    async function updateItem(e){
        ...
        try{
            const response = await axios.put(`http://3.222.129.220:80/user/edit/${id}`, updatedExpense, { headers: {"Authorization" : token }})
            showOnScreen(response.data);
    ...
    async function payment(e) {
        try{
            if(e.target.classList.contains('membership')){
                const response = await axios.get("http://3.222.129.220:80/purchase/premium", { headers: {"Authorization" : token }})
                // console.log(response.data.order.id)
                var options = {
                    "key": response.data.key_id,
                    "order_id": response.data.order.id,
                    "handler": async function (response){
                        await axios.post("http://3.222.129.220:80/purchase/updatetransactionstatus",{
        ...
        rzp1.on('payment.failed', async function (response){
            ...
            await axios.post("http://3.222.129.220:80/purchase/transactionfailstatus", response.error.metadata ,{ headers: {"Authorization" : token }})
