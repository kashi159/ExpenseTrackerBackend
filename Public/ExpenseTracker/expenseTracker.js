...
window.addEventListener("DOMContentLoaded", async() => {
    ...
    try {
      const response = await axios.get(`http://34.229.254.109:4000/expense/expense/page/?page=${page}`, {
        headers: {
          "Authorization": token,
...
async function isPremium(){
    try{
        const user = await axios.get("http://34.229.254.109:4000/user/status", { headers: {"Authorization" : token }})
        // console.log(user)
        if(user.data === true){
...
async function showLeaderBoard(e){
    ...
    try{
        ...
        const users = await axios.get("http://34.229.254.109:4000/premium/leadershipboard", { headers: {"Authorization" : token }});
        // console.log(users);
...
async function onSubmit(e) {
    ...
    try{
        const response = await axios.post('http://34.229.254.109:4000/expense/expense', userExpense,{
             headers: {
                "Authorization" : token 
            }
        });
    ...
    try {
        if(e.target.classList.contains('delete')){
            if(confirm('Are You Sure?')){
                ...
                id = li.id;
                await axios.delete(`http://34.229.254.109:4000/expense/delete/${id}`, { headers: {"Authorization" : token }})
                expense.removeChild(li);
                showTotalExpense();
            }
            }
   ...
   //Edit item//
    ...
    async function editUser(e) {
        try{
            if(e.target.classList.contains('edit')){
                var li=e.target.parentElement;
                id = li.id;
                const response = await axios.get(`http://34.229.254.109:4000/expense/edit/${id}`, { headers: {"Authorization" : token }});
                console.log(response)
                expense.removeChild(li);
    ...
    async function updateItem(e){
        ...
        try{
            const response = await axios.put(`http://34.229.254.109:4000/expense/edit/${id}`, updatedExpense, { headers: {"Authorization" : token }})
            showOnScreen(response.data);
    ...
    async function payment(e) {
        try{
            if(e.target.classList.contains('membership')){
                const response = await axios.get("http://34.229.254.109:4000/purchase/premium", { headers: {"Authorization" : token }})
                // console.log(response.data.order.id)
                var options = {
                    "key": response.data.key_id,
                    "order_id": response.data.order.id,
                    "handler": async function (response){
                        await axios.post("http://34.229.254.109:4000/purchase/updatetransactionstatus",{
        ...
        rzp1.on('payment.failed', async function (response){
            ...
            await axios.post("http://34.229.254.109:4000/purchase/transactionfailstatus", response.error.metadata ,{ headers: {"Authorization" : token }})
