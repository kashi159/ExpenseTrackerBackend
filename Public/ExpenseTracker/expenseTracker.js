...
window.addEventListener("DOMContentLoaded", async() => {
    ...
    try {
      const response = await axios.get(`http://34.229.254.109:4000/user/expense/page/?page=${page}`, {
        headers: {
          "Authorization": token,
...
async function onSubmit(e) {
    ...
    try{
        const response = await axios.post('http://34.229.254.109:4000/user/expense', userExpense,{
             headers: {
                "Authorization" : token 
            }
        });
    ...
    try {
        if(e.target.classList.contains('delete')){
            if(confirm('Are You Sure?')){
                ...
                id = li.id;
                await axios.delete(`http://34.229.254.109:4000/user/delete/${id}`, { headers: {"Authorization" : token }})
                expense.removeChild(li);
                showTotalExpense();
            }
            }
   ...
   //Edit item//
    ...
    async function editUser(e) {
        try{
            if(e.target.classList.contains('edit')){
                var li=e.target.parentElement;
                id = li.id;
                const response = await axios.get(`http://34.229.254.109:4000/user/edit/${id}`, { headers: {"Authorization" : token }});
                console.log(response)
                expense.removeChild(li);
    ...
    async function updateItem(e){
        ...
        try{
            const response = await axios.put(`http://34.229.254.109:4000/user/edit/${id}`, updatedExpense, { headers: {"Authorization" : token }})
            showOnScreen(response.data);
